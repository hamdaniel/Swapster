...
/**
 * EnumWindows callback: swaps the first WhatsApp window found.
 * Returns FALSE after handling WhatsApp to stop enumeration.
 */
...
int main() {
    ...
    std::cout << "Swapster server running. Waiting for trigger..." << std::endl;

    HANDLE triggerEvent = CreateEventW(NULL, FALSE, FALSE, L"Global\\SwapsterTriggerEvent");
    if (!triggerEvent) {
    ...
    while (true) {
        ...
        std::cout << "Trigger received. Moving WhatsApp..." << std::endl;
